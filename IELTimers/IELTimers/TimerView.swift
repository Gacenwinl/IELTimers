...
struct TimerView: View {
    ...
    var body: some View {
        VStack {
            Text("开始时间: \(startTime != nil ? formatDate(startTime!) : "未开始")")
                .font(.headline)
                .padding(.top)
    ...
    func validateAndSaveData() {
        if projectName.isEmpty, !readingCode.isEmpty {
            projectName = readingCode
        }

        if let total = Int(totalQuestions), let correct = Int(correctQuestions), correct > total {
            errorMessage = "正确题目数不能大于总题目数。"
            showErrorAlert = true
            return
        }
